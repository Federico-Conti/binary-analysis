...
L'instrumental significa modificare il codice binario per aggiungere funzionalità di monitoraggio o logging durante l'esecuzione.

Sostituire codice esistente cioè patchare è fattibile finché ciò che si vuole inserire rientra nei byte che si possono modificare. Ad esempio:

- Se un’istruzione occupa 5 byte, è possibile sostituirla con altre istruzioni purché rimangano entro i 5 byte.
- Se le nuove istruzioni sono più corte, si possono riempire i byte rimanenti con NOP (No Operation).
- Se invece sono più lunghe, si rischia di "rompere" qualcosa.

Un aspetto cruciale è considerare i target di salto. Se l’istruzione che si modifica è destinazione di un jump, è necessario rispettare gli indirizzi originali. Ad esempio:

- Con due istruzioni contigue da 2 e 3 byte:
    - Se nessuna è target di un salto, si possono modificare liberamente tutti i 5 byte.
    - Se la seconda istruzione è target di un salto, non si possono cambiare i confini (es. usare 3 byte per la prima e 2 per la seconda), altrimenti il processore interpreterà i byte in modo errato rispetto all’allineamento originale.

In generale, modificare il codice a runtime è complesso e richiede attenzione per evitare problemi legati agli indirizzi e all’allineamento delle istruzioni.