...
Ok. C’è un esercizio che farete: intanto vorrei mostrarti qualcosa. strace è un comando che permette di vedere le system call eseguite da un programma. Ad esempio tracciando check_serial ottieni molte righe dovute al loader dinamico (che puoi ignorare). L’unica vera system call eseguita dal programma può essere una write corrispondente a una printf verso stdout o stderr (ad esempio write(2, "...", 17)).

Voglio mostrarti qualcosa di sospetto: eseguo un binario debug e il programma stampa “I’m evil”; se provo a capire cosa fa ottengo “hello world”. Lo stesso accade con un binario anti-debug: quando lo eseguo sotto gdb o se è tracciato con strace il comportamento è diverso — il programma rileva di essere osservato e cambia comportamento. Questo è usato per simulare come fanno alcuni malware: se il malware capisce di essere in un ambiente di analisi (sandbox, debugger), non esegue la payload malevola, così elude l’analisi; quando è eseguito su una macchina reale, esegue il danno (es. cifratura file). L’esercizio ti permetterà di giocare con questi esempi; torneremo su questo venerdì. Ci sono altri due esempi: lo spoiler è che il terzo si risolve con gli strumenti visti finora, mentre gli altri richiedono strumenti più avanzati che vedremo più avanti.
